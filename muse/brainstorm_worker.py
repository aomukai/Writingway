...
class BrainstormWorker(QThread):
    ...
    def run(self):
        ...
        for group in self.run_groups:
            ...
            for i in range(group.runs):
                if self.is_cancelled:
                    break
                
                self.progress_updated.emit(int((completed_runs / total_runs) * 100), f"Starting run {i+1}/{group.runs} for {group.provider} - {group.model}")
                ...
                while retry_count <= group.retries:
                    try:
                        ...
                        if retry_count > group.retries:
                            self.error.emit(f"Error during run {i+1} for {group.provider} - {group.model}: {e}")
                            break
